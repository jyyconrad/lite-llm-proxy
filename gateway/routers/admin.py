...
from ..dependencies import authenticate_user, get_db_session, incr_rate_limit, require_admin
...
# ---- 用户管理 ----
@router.post("/users")
async def create_user(uc: UserCreate, admin=Depends(require_admin), db=Depends(get_db_session)):
    ...
    try:
        ...
        new_key = APIKey(id=str(uuid.uuid4()), api_key=api_key_val, user_id=uid, description="自动生成的API密钥（管理员创建）")
        db.add(new_key)
        await db.commit()
    except Exception:
        await db.rollback()
        ...
        }, "api_key": None, "warning": "用户已创建，但生成 API key 失败"}
...
async def create_key(ak: APIKeyCreate, admin=Depends(require_admin), db=Depends(get_db_session)):
    key_val = f"sk-{randstr(8)}"
    db.add(APIKey(id=str(uuid.uuid4()), api_key=key_val, user_id=ak.user_id, description=ak.description))
    await db.commit()
...
async def get_key(key_id: str, admin=Depends(require_admin), db=Depends(get_db_session)):
    """获取特定API密钥信息"""
...
async def disable_key(key_id: str, admin=Depends(require_admin), db=Depends(get_db_session)):
    """禁用API密钥"""
    key = await db.get(APIKey, key_id)
    if not key:
        raise HTTPException(status_code=404, detail="API key not found")
...
async def enable_key(key_id: str, admin=Depends(require_admin), db=Depends(get_db_session)):
    """启用API密钥"""
    key = await db.get(APIKey, key_id)
    if not key:
        raise HTTPException(status_code=404, detail="API key not found")
...
async def delete_key(key_id: str, admin=Depends(require_admin), db=Depends(get_db_session)):
    """删除API密钥"""
    key = await db.get(APIKey, key_id)
    if not key:
        raise HTTPException(status_code=404, detail="API key not found")
...
async def get_user_key(user_id: str, admin=Depends(require_admin), db=Depends(get_db_session)):
    """管理员专用：获取指定用户的最新可用 API 密钥值。"""
    # 查询该用户最新创建且仍然存在的 API Key（按创建时间降序）
    stmt = select(APIKey).where(APIKey.user_id == user_id).order_by(APIKey.created_at.desc()).limit(1)
    result = await db.execute(stmt)
...
async def stats_overview(u=Depends(authenticate_user),db=Depends(get_db_session)):
    ...
    # 总调用次数
    total_calls = (
        await db.execute(select(func.sum(UsageStat.request_count)).where(*where_clauses))
    ).scalar() or 0
    ...
    total_cost = (
        (await db.execute(select(func.sum(UsageStat.total_cost)).where(*where_clauses))).scalar() or 0
    )
...
async def model_usage_stats(u=Depends(authenticate_user), db=Depends(get_db_session)):
    ...
    ).where(*where_clauses).group_by(UsageStat.model_name)
...
async def recent_activity(limit: int = 10, u=Depends(authenticate_user), db=Depends(get_db_session)):
    """最近活动记录"""
    ...
    ).where(*where_clauses).group_by(UsageStat.user_id, UsageStat.model_name).order_by(func.max(UsageStat.last_used).desc()).limit(limit)
    ...
    for row in results:
        ...
        activities.append({
            ...
            "timestamp": row.last_used.isoformat() if row.last_used else datetime.utcnow().isoformat(),
...
async def user_stats(user_id: str, db=Depends(get_db_session)):
    ...
    return {
        ...
        "last_activity": usage_row.last_activity.isoformat() if usage_row.last_activity else None,
...
async def usage_trend(
    ...
    # 根据粒度确定分组表达式（针对 CompletionLog.created_at）
    if granularity == "day":
        group_by_expr = func.date(CompletionLog.created_at)
    elif granularity == "week":
        # week/year concat - fallback
        group_by_expr = func.concat(func.extract('year', CompletionLog.created_at), '-', func.extract('week', CompletionLog.created_at))
    elif granularity == "month":
        group_by_expr = func.concat(func.extract('year', CompletionLog.created_at), '-', func.extract('month', CompletionLog.created_at))
    else:
        group_by_expr = func.date(CompletionLog.created_at)
    ...
    # 构建 where 条件（基于 CompletionLog）
    where_clauses = [CompletionLog.created_at >= start_date, CompletionLog.created_at <= end_date]
    if model:
        where_clauses.append(CompletionLog.model_name == model)
    if user_id:
        where_clauses.append(CompletionLog.user_id == user_id)

    # 查询并按日期聚合 CompletionLog（每条日志代表一次请求）
    stmt = select(
        group_by_expr.label("date"),
        func.count(CompletionLog.id).label("calls"),
        func.sum(CompletionLog.total_tokens).label("tokens"),
        func.sum(CompletionLog.cost).label("cost"),
        func.count(func.distinct(CompletionLog.user_id)).label("users")
    ).where(*where_clauses).group_by(group_by_expr).order_by(group_by_expr)
    ...
    for stat in stats_data:
        data.append({
            "date": str(stat[0]),  # 第一个字段是日期
            "calls": int(stat[1] or 0),  # 第二个字段是调用次数
            "tokens": int(stat[2] or 0),  # 第三个字段是token数
            "cost": float(stat[3] or 0),  # 第四个字段是成本
            "users": int(stat[4] or 0)  # 第五个字段是用户数
        })
...
async def user_trend(
    ...
    if granularity == "day":
        group_by_expr = func.date(CompletionLog.created_at)
        select_fields = [group_by_expr.label("date")]
        order_by_field = group_by_expr
    elif granularity == "week":
        # week/year concat - fallback
        group_by_expr = func.concat(func.extract('year', CompletionLog.created_at), '-', func.extract('week', CompletionLog.created_at))
        select_fields = [group_by_expr.label("week")]
        order_by_field = group_by_expr
    elif granularity == "month":
        group_by_expr = func.concat(func.extract('year', CompletionLog.created_at), '-', func.extract('month', CompletionLog.created_at))
        select_fields = [group_by_expr.label("month")]
        order_by_field = group_by_expr
    else:
        group_by_expr = func.date(CompletionLog.created_at)
        select_fields = [group_by_expr.label("date")]
    ...
    stmt = select(
        ...
        *select_fields,
        ...
        func.sum(CompletionLog.total_tokens).label("tokens"),
        func.sum(CompletionLog.cost).label("cost")
    ).where(*where_clauses).group_by(CompletionLog.user_id, group_by_expr).order_by(CompletionLog.user_id, order_by_field)
    
    result = await db.execute(stmt)
    stats_data = result.all()
    
    # 获取用户信息
    user_ids = list(set([stat.user_id for stat in stats_data]))
    ...
    data = []
    for stat in stats_data:
        username = user_map.get(stat.user_id, f"User {stat.user_id[:8]}")
        data_entry = {
            "user_id": stat.user_id,
            "username": username,
            "calls": int(stat.calls or 0),
        ...
        # 添加时间字段
        if granularity == "day":
            data_entry["date"] = str(stat.date) if hasattr(stat, 'date') else ""
        elif granularity == "week":
            data_entry["week"] = str(stat.week) if hasattr(stat, 'week') else ""
        elif granularity == "month":
            data_entry["month"] = str(stat.month) if hasattr(stat, 'month') else ""
...
async def model_trend(
    ...
    stmt = select(
        ...
        *select_fields,
        ...
        func.sum(CompletionLog.total_tokens).label("tokens"),
        func.sum(CompletionLog.cost).label("cost")
    ).where(*where_clauses).group_by(CompletionLog.model_name, group_by_expr).order_by(CompletionLog.model_name, order_by_field)
