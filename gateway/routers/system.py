...
from ..dependencies import hash_password, verify_password, get_db_session, authenticate_user, require_admin
...
@router.get("/users")
async def get_users(page: int = 1, per_page: int = 20, admin=Depends(require_admin), db=Depends(get_db_session)):
    """获取所有用户列表（仅管理员），支持分页"""
    if page < 1:
        page = 1
...
async def update_user(user_id: str, user_data: dict, admin=Depends(require_admin), db=Depends(get_db_session)):
    """更新用户信息（仅管理员）"""
    ...
    # 更新允许修改的字段
    allowed_fields = ["username", "email", "role", "budget_limit", "rpm_limit", "tpm_limit", "is_active"]
...
async def enable_user(user_id: str, admin=Depends(require_admin), db=Depends(get_db_session)):
    user = await db.get(User, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="用户不存在")
    user.is_active = True
...
async def disable_user(user_id: str, admin=Depends(require_admin), db=Depends(get_db_session)):
    user = await db.get(User, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="用户不存在")
...
async def reset_user_password(user_id: str, password_data: dict, admin=Depends(require_admin), db=Depends(get_db_session)):
    """重置用户密码（仅管理员）"""
    user = await db.get(User, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="用户不存在")
...
async def create_user_api_key(user_id: str, key_data: dict, admin=Depends(require_admin), db=Depends(get_db_session)):
    """为用户创建新的API密钥（仅管理员）"""
    user = await db.get(User, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="用户不存在")
...
async def create_own_api_key(key_data: dict = {}, user=Depends(authenticate_user), db=Depends(get_db_session)):
    """当前登录用户为自己创建 API Key"""
...
async def list_own_api_keys(user=Depends(authenticate_user), db=Depends(get_db_session)):
    """列出当前登录用户的 API keys"""
    stmt = select(APIKey).where(APIKey.user_id == user["user_id"]).order_by(APIKey.created_at.desc())
    result = await db.execute(stmt)
...
async def disable_own_key(key_id: str, user=Depends(authenticate_user), db=Depends(get_db_session)):
    """用户禁用自己的 API Key"""
...
async def enable_own_key(key_id: str, user=Depends(authenticate_user), db=Depends(get_db_session)):
    """用户启用自己的 API Key"""
    key = await db.get(APIKey, key_id)
    if not key:
        raise HTTPException(status_code=404, detail="API key not found")
