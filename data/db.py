...
def _ensure_engine():
    ...
    sync_engine_inst = sync_engine(sync_url, pool_pre_ping=True)
    # 设置时区为Asia/Shanghai
    with sync_engine_inst.connect() as conn:
...
async def _ensure_admin():
    ...
    async with AsyncSessionLocal() as s:
        ...
        if admin is None:
            ...
            s.add(User(id="admin001", username="admin",
                       email="admin@litellm-gateway.com", role="admin",
                       password_hash=password_hash,
                       budget_limit=10_000, rpm_limit=1000, tpm_limit=1_000_000))
            s.add(APIKey(id="admin-key-001", api_key=get_settings().master_key,
                         user_id="admin001", description="Master admin key"))
            await s.commit()
